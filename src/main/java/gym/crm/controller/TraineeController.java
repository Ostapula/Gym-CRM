...
public class TraineeController {
    ...
    public ResponseEntity<?> createTrainee(
        ...
        if (traineeDto.getFirstName() == null || traineeDto.getLastName() == null) {
            return new ResponseEntity<>(new ErrorMessage(400,
                    "Bad Request",
    ...
    @GetMapping
    public ResponseEntity<?> getTrainee(@RequestParam String username) {
        TraineeDto traineeDto = traineeService.getTraineeByUsername(username).orElse(null);
        if (traineeDto == null) {
            return new ResponseEntity<>(new ErrorMessage(404, "Not Found", "Trainee not found"), HttpStatus.NOT_FOUND);
        }
    ...
    public ResponseEntity<?> updateTrainee(
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "Trainee fields to update", required = true,
                    content = @Content(schema = @Schema(implementation = TraineeDto.class)))
            @RequestBody TraineeDto traineeDto) {
        if (traineeDto.getUsername() == null || traineeDto.getFirstName() == null || traineeDto.getLastName() == null) {
    ...
    public ResponseEntity<?> assignTrainer(@PathVariable String username,
        ...
        if (trainerUsernames == null || trainerUsernames.isEmpty()) {
            return new ResponseEntity<>(new ErrorMessage(400, "Bad Request", "Trainers list is required"), HttpStatus.BAD_REQUEST);
        }
